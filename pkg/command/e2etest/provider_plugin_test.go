...
func TestProviderProtocols(t *testing.T) {
	...
	simple6ProviderExe := e2e.GoBuild("github.com/adrien-f/opentofu-opened/pkg/provider-simple-v6/main", simple6Provider)

	simpleProvider := filepath.Join(tf.WorkDir(), "terraform-provider-simple")
	simpleProviderExe := e2e.GoBuild("github.com/adrien-f/opentofu-opened/pkg/provider-simple/main", simpleProvider)
